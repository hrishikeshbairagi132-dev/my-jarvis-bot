...
st.markdown("---")
st.markdown("*GNews API • Telegram Alerts • Realistic Backtest • Paper Trading • Mobile-Friendly • Secure*")
...
# ============================================================
# 🔒 SECURE API Keys (st.secrets) — GitHub Safe
# ============================================================
try:
    ...
    TELEGRAM_CHAT_ID = ""
    GNEWS_API_KEY = ""
    st.warning("⚠️ Secrets not configured. Telegram alerts & News will be disabled.")

# ============================================================
# Telegram Alert Function (Secure)
# ============================================================
...
# ============================================================
# News Sentiment Function (Secure Fallback)
# ============================================================
...
def fetch_news_sentiment(raw_ticker: str):
    ...
    neg_words = ["loss","drop","slump","bearish","downgrade","fraud","decline","risk","fail","sell","negative","crash","debt","plaintiff","breach"]
    
    if not GNEWS_API_KEY:
        return fetch_yfinance_news(raw_ticker, pos_words, neg_words)
    
    try:
        url = f"https://gnews.io/api/v4/search?q={raw_ticker}&lang=en&country=in&max=5&apikey={GNEWS_API_KEY}"
        ...
        final_score = round(max(-1, min(1, score / n)), 2)
        return final_score, headlines[:5]
    except Exception:
        return fetch_yfinance_news(raw_ticker, pos_words, neg_words)

def fetch_yfinance_news(raw_ticker, pos_words, neg_words):
    try:
...
with st.sidebar:
    ...
    ticker_input = st.text_input("📌 टिकर (TATAMOTORS, RELIANCE, BTC-USD)", "TATAMOTORS").upper().strip()
    capital = st.number_input("💰 कुल पूंजी (₹)", min_value=500.0, value=50000.0, step=5000.0)
    max_risk_pct = st.slider("⚠️ अधिकतम जोखिम (capital %)", min_value=0.5, max_value=5.0, value=1.0, step=0.5)
    refresh_bucket = st.selectbox("🔄 Refresh bucket (sec)", [10, 30, 60], index=1)
    news_bucket = st.selectbox("📰 News bucket (sec)", [60, 120, 300], index=1)
    asset_class = st.selectbox("🏛️ Asset Class", ["Equity (NSE)", "Crypto"], index=0)
    
    st.markdown("---")
    
    if TELEGRAM_BOT_TOKEN and GNEWS_API_KEY:
        st.success("✅ GNews API ON • Telegram Alerts ON")
    else:
        st.warning("⚠️ Secrets not configured • Limited functionality")
...
def detect_patterns_rolling(df):
    df = df.copy()
...
if st.button("🚀 विश्लेषण शुरू करें", use_container_width=True):
    ok, msg = check_market_hours(asset_class)
    if not ok:
        st.warning(msg)
        st.stop()
...
"
    ...
    m3.metric("ADX (14)", f"{last['adx']:.1f}")
    m4.metric("ATR (14)", f"₹{last['atr']:.2f}")
