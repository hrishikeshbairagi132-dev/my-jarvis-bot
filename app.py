    ...
    if nav == "📊 Dashboard":
                ...
                if "BUY" in sig_type or "SELL" in sig_type:
                    if st.button("🔍 Validate Signal 10x"):
                        ...
                        def progress_callback(iter_num):
                            progress_bar.progress(iter_num / 10.0)
                    ...
                    st.info("⏳ Status: HOLD (No high-probability setups detected)")
        ...
        with st.expander("Place Order", True):
            ...
            if st.button("Execute Trade", use_container_width=True):
                if qty<=0 or price<=0: st.error("Invalid qty/price")
                    ...
                    if ok: 
                        sl_mock = price * 0.98 if side == "BUY" else price * 1.02
                        tp_mock = price * 1.05 if side == "BUY" else price * 0.95
                        log_trade(ticker, side, price, sl_mock, tp_mock, qty, 0.1, "Dashboard Executed")
                        st.success(msg); safe_rerun()
                    else: st.error(msg)
        ...
        with col1:
            ...
            for pos in st.session_state.paper_positions:
                ...
                color = "#00ffcc" if pnl >= 0 else "#ff4b4b"
                st.markdown(f"<div class='trade-card'><b>{pos['ticker']}</b> Qty:{pos['qty']} Entry:₹{pos['buy_price']:.2f} | Current:₹{pos['current_price']:.2f} | <span style='color:{color}; font-weight:bold;'>P&L: ₹{pnl:.2f}</span></div>", unsafe_allow_html=True)
        ...
        for i,item in enumerate(st.session_state.watchlist):
            ...
            live_val = _fetch_uncached(item)
            c1.write(f"• **{item}** — Price: ₹{live_val if live_val else 'N/A'}")
        ...
        st.caption("App Version: 2.1.0-Stable (Modular Core)")
