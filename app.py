...
def fetch_data(ticker: str, period: str = "60d"):
    try:
        ...
        if df is None or df.empty:
            return None, f"Empty data from Yahoo for {sym}"
        ...
        if len(df) < 30:
            return None, f"Not enough rows after indicators: {len(df)}"
...
def set_error(message: str, debug: str = ""):
    st.session_state.last_error = message
    st.session_state.debug_info = debug
    st.session_state.analysis_done = False
    st.session_state.result = None
    st.session_state.status_box = message


def run_analysis():
    try:
        ticker_input = st.session_state.ticker_input.strip().upper()
        capital = float(st.session_state.capital)
        risk_pct = float(st.session_state.risk_pct)

        df, status = fetch_data(ticker_input, period="60d")
        if df is None or df.empty:
            set_error("No data received from Yahoo Finance.", f"Ticker={ticker_input}, status={status}")
            return
        ...
        st.session_state.debug_info = f"Ticker={ticker_input}, rows={len(df)}, signal={signal}, news={news_score}"
...
Conf: {conf:.1f}%"""
            ...
            if not ok:
                st.session_state.debug_info += f" | TelegramError={err}"
            else:
                log_trade([
        ...
        set_error("Unexpected error occurred.", traceback.format_exc())
