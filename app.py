...
def run_analysis():
    ticker_input = st.session_state.get("ticker_input", "TATAMOTORS").upper().strip()
    capital = st.session_state.get("capital", 50000.0)
...
with st.sidebar:
    st.text_input("Ticker", "TATAMOTORS", key="ticker_input")
    st.number_input("Capital", min_value=500.0, value=50000.0, step=1000.0, key="capital")
    st.slider("Risk %", min_value=0.5, max_value=5.0, value=1.0, step=0.5, key="risk_pct")
